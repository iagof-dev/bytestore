    ...
    if (mysqli_num_rows($anuncios) > 0) {
        ...
        while ($linha = mysqli_fetch_assoc($anuncios)) {
            if ($qnt_anuncios <= 5){
                ...
                $txt= mb_strimwidth($linha["description"], 0, 200, "...");
                $title_long = mb_strimwidth($linha["title"], 0, 32, "...");
                $anuncio = '<div class="row position-relative" data-aos="zoom-in" data-aos-delay="'. $delay .'" data-aos-duration="1000" style="padding-top: 1% !important;"><div class="col"><img width="100px" height="100px"                 src="./Assets/imgs/products/'. $linha["image"]. '">             </div>             <div class="col-5 text-start">               <h1 style="margin-bottom: -10px !important;">'. $title_long. '</h1>               <p>'. $txt. '</p>             </div>              <div class="col">               <div style="margin-top: -10%;" class="text-center position-relative top-50 start-50 translate-middle">                 <h2>R$ '. $linha["price"]. '</h2>               </div>             </div>              <div class="col">               <div style="margin-top: 20%;" class="infos position-relative text-center">                   <div class="dropdown"> <button class="btn btn-primary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false"> <i class="fa-solid fa-pen" style="height: 5px;"></i> Modificar </button><ul class="dropdown-menu"><li><a class="dropdown-item" href="/product?id='. $linha["id"]. '"><i class="fa-solid fa-eye"></i> Ver anúncio</a></li><li><a class="dropdown-item" href="/modify?id='. $linha["id"]. '&file='. $linha["image"] .'"><i class=" fa-solid fa-pen-to-square"></i> Editar</a></li><li><a class="dropdown-item" href="/delete?id='. $linha["id"]. '&file=' . $linha['image'] . '"><i class="fa-solid fa-trash-can"></i> Deletar</a></li></ul></div></div></div></div> ';
        ...
        return [$qnt_anuncios,$all_anuncios];

    }
    else{
        return [0, '<h1 style="color: red; padding-top: 10px;" class="center">Sem anúncios para mostrar :(</h1>'];
...
function get_all_purchases(){
    ...
    if (mysqli_num_rows($getpurchases) > 0) {
        while($linha = mysqli_fetch_assoc($getpurchases)){
            $long_title = mb_strimwidth($linha['title'], 0, 29, '...');
            $count = $count + 1;
            $delay += 300;
                if($delay >= 1500){
                    $delay = 300;
            ...
            switch($linha['status']){
                case 'accredited':
                    $retornar = $retornar . '<div class="compra" data-aos="zoom-in" data-aos-delay="'. $delay .'" data-aos-duration="1000"> <div class="row"> <div class="col justify-content-start"> <img width="50%" src="../Assets/imgs/products/'. $linha['image'] .'"> </div> <div class="col-5 justify-content-center"> <h1 class="pur-title text-start">'. $long_title .'</h1> <h2 class="detail text-start">#'. $linha['payment_id'] .' | Metódo: '. $linha['payment_method'] .'</h2> </div> <div class="col text-center justify-content-end"> <div class="infos"> <div class="col text-center"> <h3 class="">R$'. $linha['value'] .'</h3><br> </div> <div class="col text-center"> <h2 class="text-center center text-status text-success">Aprovado</h2> </div> <div class="col text-center"><a href="/details?id='. $linha['payment_id'] .'&img='. $linha['image'] .'&product_id='. $linha['id'] .'"><button class="btn btn-primary "> <i class="fa-solid fa-circle-info"></i></button></a> </div> </div> </div> </div> </div>';
                    break;
                case 'approved':
                    $retornar = $retornar . '<div class="compra" data-aos="zoom-in" data-aos-delay="'. $delay .'" data-aos-duration="1000"> <div class="row"> <div class="col justify-content-start"> <img width="50%" src="../Assets/imgs/products/'. $linha['image'] .'"> </div> <div class="col-5 justify-content-center"> <h1 class="pur-title text-start">'. $long_title .'</h1> <h2 class="detail text-start">#'. $linha['payment_id'] .' | Metódo: '. $linha['payment_method'] .'</h2> </div> <div class="col text-center justify-content-end"> <div class="infos"> <div class="col text-center"> <h3 class="">R$'. $linha['value'] .'</h3><br> </div> <div class="col text-center"> <h2 class="text-center center text-status text-success">Aprovado</h2> </div> <div class="col text-center"><a href="/details?id='. $linha['payment_id'] .'&img='. $linha['image'] .'&product_id='. $linha['id'] .'"><button class="btn btn-primary "> <i class="fa-solid fa-circle-info"></i></button></a> </div> </div> </div> </div> </div>';
                    break;
